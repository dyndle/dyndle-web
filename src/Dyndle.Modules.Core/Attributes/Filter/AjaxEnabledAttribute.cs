...
namespace Dyndle.Modules.Core.Attributes.Filter
{
    ...
    public class AjaxEnabledAttribute : ActionFilterAttribute
    {
        ...
        /// <exception cref="ArgumentNullException">Cannot determine area from route data, ensure that either an area or controller segment is present in the URL</exception>
        public override void OnActionExecuting(ActionExecutingContext filterContext)
        {
            if (!filterContext.RouteData.DataTokens.ContainsKey("area"))
            {
                ...
                {
                    throw new ArgumentNullException("filterContext", "Cannot determine area from route data, ensure that either an area or controller segment is present in the URL");
                }
            }
        }
    }
